...
if option == "ボーナス確率のみ":
    ...
    if st.button("実行"):
        ...
        df_probabilities = pd.DataFrame({
            "BB":[row.BB_flag_counts/row.amount_flag_counts for row in df_flag_counts.itertuples(index=False, name="setting")],
            "RB":[row.RB_flag_counts/row.amount_flag_counts for row in df_flag_counts.itertuples(index=False, name="setting")],
            "ハズレ":[(row.amount_flag_counts - row.BB_flag_counts - row.RB_flag_counts)/row.amount_flag_counts for row in df_flag_counts.itertuples(index=False, name="setting")]
        })
        df_probabilities_include_grapes = pd.DataFrame({
            ...
            "ハズレ":[(row.amount_flag_counts - row.BB_flag_counts - row.RB_flag_counts - row.grapes_flag_counts)/row.amount_flag_counts for row in df_flag_counts.itertuples(index=False, name="setting")]
        })
